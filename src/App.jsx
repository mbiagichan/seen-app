...
async function searchMovies(query) {
  ...
  try {
    // Search both movies and people simultaneously
    ...
    // Check if the top movie result is a strong title match
    const queryLower = query.trim().toLowerCase();
    const topMovie = movieResults[0];
    const topMovieTitleLower = topMovie?.title?.toLowerCase() || "";
    const strongMovieMatch = topMovie && (
      topMovieTitleLower === queryLower ||
      topMovieTitleLower.startsWith(queryLower) ||
      queryLower.startsWith(topMovieTitleLower) ||
      (topMovie.popularity && topMovie.popularity > 20)
    );

    // Only use person search if:
    // 1. A person was found
    // 2. The person's name closely matches the query (not just a coincidence)
    // 3. There is NOT a strong movie title match
    const topPerson = personResults[0];
    const topPersonNameLower = topPerson?.name?.toLowerCase() || "";
    const strongPersonMatch = topPerson && (
      topPersonNameLower === queryLower ||
      topPersonNameLower.includes(queryLower) ||
      queryLower.includes(topPersonNameLower.split(" ")[0]) // first name match
    );

    if (strongPersonMatch && !strongMovieMatch && topPerson.known_for_department) {
    ...
    // Default: return movie title results sorted by popularity
    return movieResults
      .sort((a, b) => (b.popularity || 0) - (a.popularity || 0))
      .slice(0, 12)
      .map(movie => ({
