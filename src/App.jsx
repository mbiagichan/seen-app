...
function FeedItem({ item, liked, onLike, comments, onAddComment, T, onMovieClick, onEdit }) {
    ...
    <div style={{ background: T.card, borderRadius: 16, marginBottom: 14, border: `1px solid ${T.border}`, overflow: "hidden", transition: "transform 0.2s, box-shadow 0.2s" }}>
      <div style={{ padding: "14px 16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: T.text }}>{item.user.name}</div>
            ...
            <>
              <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
                <input
                  ...
                  autoFocus
                  style={{ flex: 1, padding: "13px 16px", background: T.bg, border: `1.5px solid ${T.borderMed}`, borderRadius: 12, color: T.text, fontSize: 15, outline: "none" }}
                />
                <button onClick={doSearch} disabled={loading} style={{ padding: "13px 22px", background: T.accent, border: "none", borderRadius: 12, color: "#fff", fontWeight: 700, fontSize: 14, cursor: loading ? "wait" : "pointer", opacity: loading ? 0.7 : 1, transition: "opacity 0.2s" }}>
                  {loading ? "..." : "Search"}
                </button>
              </div>
              ...
              <div style={{ display: "flex", gap: 14, marginBottom: 24, padding: 16, background: T.bg, borderRadius: 14 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {selectedMovie.overview && (
                    <div style={{ fontSize: 12, color: T.textSec, lineHeight: 1.4, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical" }}>
                      {selectedMovie.overview}
                    </div>
                  )}
                </div>
...
function DiscoverTab({ onCheckin, T, onMovieClick }) {
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {movieResults.map((movie, i) => (
            <div
              ...
              style={{ cursor: "pointer", transition: "transform 0.2s, opacity 0.2s", animation: `fadeIn 0.4s ease-out ${i * 0.05}s backwards` }}
              onMouseEnter={e => e.currentTarget.style.transform = "translateY(-4px)"}
              onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}
            >
              <div style={{ aspectRatio: "2/3", background: T.bg, border: `1px solid ${T.border}`, borderRadius: 12, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
                {movie.poster ? (
                  <img src={movie.poster} alt={movie.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                ) : (
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <input
          ...
          onKeyDown={e => e.key === "Enter" && subTab === "movies" && doMovieSearch()}
          placeholder={
        ...
        />
        {subTab === "movies" && (
          <button onClick={doMovieSearch} disabled={loading} style={{ padding: "12px 20px", background: T.accent, border: "none", borderRadius: 12, color: "#fff", fontWeight: 700, fontSize: 14, cursor: loading ? "wait" : "pointer", opacity: loading ? 0.7 : 1 }}>
            {loading ? "..." : "Search"}
          </button>
        )}
        {subTab === "collections" && (
      ...
      {/* DM Modal */}
      {showDM && (
        <DMModal user={showDM} onClose={() => setShowDM(null)} T={T} />
      )}
    </div>
  );
}
...
// --- DM Modal ---
function DMModal({ user, onClose, T }) {
  ...
  const handleSend = () => {
    ...
    console.log("Sending DM to", user.name, ":", message, selectedMovie);
    onClose();
  };
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: T.card, width: "100%", maxWidth: 480, margin: "0 auto", borderRadius: "24px 24px 0 0", maxHeight: "80vh", overflow: "hidden", display: "flex", flexDirection: "column", animation: "slideUp 0.3s ease-out" }}>
        ...
        <div style={{ flex: 1, padding: "20px", overflow: "auto" }}>
          <div style={{ textAlign: "center", padding: "40px 20px", color: T.textTer }}>
            <div style={{ fontSize: 14, marginBottom: 8 }}>Send a message to {user.name}</div>
...
function ProfileTab({ checkins, stats, T, darkMode, userCollections, onMovieClick, onEditCheckin, onDeleteCheckin, onAddToCollection, onRemoveFromCollection, onEditCollection, onDeleteCollection, onEdit }) {
  ...
  return (
    ...
    <div>
      ...
      <div style={{ textAlign: "center", padding: "28px 0 20px" }}>
        ...
        <div style={{ fontSize: 21, fontWeight: 800, color: T.text, marginTop: 12, letterSpacing: -0.4 }}>You</div>
      ...
      {/* Sub-tab Navigation */}
      <div style={{ display: "flex", gap: 8, marginBottom: 16, borderBottom: `1px solid ${T.border}`, paddingBottom: 2, overflowX: "auto" }}>
        {subTabs.map(({ id, label, icon: Icon }) => (
          <button
            key={id}
        ...
        <div>
          ...
          {checkins.length === 0 ? (
            <div style={{ textAlign: "center", color: T.textTer, padding: 48, fontSize: 13, background: T.card, borderRadius: 16, border: `1px solid ${T.border}` }}>
              <TicketIcon size={32} color={T.textTer} />
...
function EditCheckinModal({ checkin, onClose, onSave, onDelete, T, userCollections, onAddToCollection, onRemoveFromCollection }) {
  ...
  const handleDelete = () => {
    ...
    if (window.confirm(`Are you sure you want to delete this check-in for "${checkin.movieTitle}"? This action cannot be undone.`)) {
...
// --- Movie Detail Modal ---
function MovieDetailModal({ movie, onClose, userCollections, onAddToCollection, onRemoveFromCollection, T, checkins }) {
  const [showCollectionPicker, setShowCollectionPicker] = useState(false);
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "20px" }}>
          ...
          <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {movie.runtime && (
                <div style={{ fontSize: 12, color: T.textSec, display: "flex", alignItems: "center", gap: 4 }}>
                  <Clock size={12} />
                  {movie.runtime} min
                </div>
              )}
...
export default function SeenApp() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // Load check-ins
        const savedCheckins = await window.storage.get('seen-checkins');
        if (savedCheckins) {
        ...
        // Load collections
        const savedCollections = await window.storage.get('seen-collections');
        if (savedCollections) {
          ...
          await window.storage.set('seen-collections', JSON.stringify(defaultCollections));
        }

        // Load dark mode preference
        const savedTheme = await window.storage.get('seen-darkmode');
        if (savedTheme) {
  ...
  useEffect(() => {
    if (!isLoading && checkins.length > 0) {
      window.storage.set('seen-checkins', JSON.stringify(checkins)).catch(err => console.log('Save error:', err));
    }
  }, [checkins, isLoading]);
  ...
  useEffect(() => {
    if (!isLoading && userCollections.length > 0) {
      window.storage.set('seen-collections', JSON.stringify(userCollections)).catch(err => console.log('Save error:', err));
    }
  ...
  useEffect(() => {
    if (!isLoading) {
      window.storage.set('seen-darkmode', darkMode.toString()).catch(err => console.log('Save error:', err));
    }
  }, [darkMode, isLoading]);
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div style={{ padding: "14px 14px 100px" }}>
            ...
            {tab === "search" && <DiscoverTab onCheckin={() => setShowCheckin(true)} T={T} onMovieClick={setSelectedMovieDetail} />}

            {tab === "profile" && <ProfileTab checkins={my} stats={stats} T={T} darkMode={darkMode} userCollections={userCollections} onMovieClick={setSelectedMovieDetail} onEditCheckin={handleEditCheckin} onDeleteCheckin={handleDeleteCheckin} onAddToCollection={addToCollection} onRemoveFromCollection={removeFromCollection} onEditCollection={editCollection} onDeleteCollection={deleteCollection} onEdit={setEditingCheckin} />}
          </div>
