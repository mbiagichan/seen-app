...
async function searchMovies(query) {
  ...
  try {
    ...
    // A "real" person result: name matches query closely AND they are popular (known actor/director)
    const topPerson = personResults[0];
    const isRealPersonSearch = topPerson &&
      topPerson.popularity > 5 && // Must be reasonably well-known
      topPerson.known_for_department && // Must have a department (acting, directing, etc.)
      topPerson.known_for && topPerson.known_for.length > 0 && // Must have known works
      (
        topPersonNameMatches(topPerson.name, queryLower)
      );

    if (isRealPersonSearch) {
      // Fetch full credits for this person
      const creditsResponse = await fetch(
        `${TMDB_BASE_URL}/person/${topPerson.id}/movie_credits?api_key=${TMDB_API_KEY}`
      );
    ...
    // Default: movie title search, sorted by popularity
    return movieResults
      .sort((a, b) => (b.popularity || 0) - (a.popularity || 0))
      .slice(0, 12)
      .map(movie => ({
...
// Helper: check if a person's name meaningfully matches the search query
function topPersonNameMatches(personName, queryLower) {
  const nameLower = personName.toLowerCase();
  const nameParts = nameLower.split(" ");
  const queryParts = queryLower.split(" ");
  // Full name match, or all query words appear in the name
  if (nameLower === queryLower) return true;
  if (nameLower.includes(queryLower)) return true;
  if (queryParts.every(part => nameParts.some(np => np.startsWith(part)))) return true;
  // At least last name matches and query has 2+ words (like "brad pitt")
  if (queryParts.length >= 2 && nameParts.some(np => queryParts.some(qp => np === qp))) return true;
  return false;
}
...
// --- DM Modal ---
function DMModal({ user, onClose, T, userDMs = {}, onSendDM, onDeleteMessage }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: T.overlay, zIndex: 100, display: "flex", alignItems: "flex-end", animation: "fadeIn 0.2s" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.card, width: "100%", maxWidth: 480, margin: "0 auto", borderRadius: "24px 24px 0 0", maxHeight: "80vh", overflow: "hidden", display: "flex", flexDirection: "column", animation: "slideUp 0.3s ease-out" }}>
        ...
        <div style={{ flex: 1, padding: "20px", overflow: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
          {conversation.length === 0 ? (
            ...
            conversation.map((msg, i) => (
              <div key={msg.id} style={{ display: "flex", justifyContent: msg.from === "me" ? "flex-end" : "flex-start", animation: `fadeIn 0.3s ease-out ${i * 0.05}s backwards`, position: "relative" }}>
        ...
        {/* Delete confirmation */}
        {deletingMsg && (
          <div style={{ position: "absolute", inset: 0, background: T.overlay, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "24px 24px 0 0" }} onClick={() => setDeletingMsg(null)}>
            <div onClick={e => e.stopPropagation()} style={{ background: T.card, padding: "24px", borderRadius: 16, maxWidth: 320, margin: "0 20px" }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: T.text, marginBottom: 8 }}>Delete message?</div>
              <div style={{ fontSize: 13, color: T.textSec, marginBottom: 20 }}>This cannot be undone.</div>
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setDeletingMsg(null)} style={{ flex: 1, padding: "10px", background: T.bg, border: `1.5px solid ${T.border}`, borderRadius: 10, color: T.text, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Cancel</button>
                <button onClick={() => handleDelete(deletingMsg.id)} style={{ flex: 1, padding: "10px", background: T.red, border: "none", borderRadius: 10, color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Delete</button>
              </div>
            </div>
          </div>
        )}
...
// --- New Message Modal ---
function NewMessageModal({ onClose, onSend, T }) {
  ...
  const toggleRecipient = (friend) => {
    setSelectedRecipients(prev =>
      prev.find(r => r.id === friend.id)
        ? prev.filter(r => r.id !== friend.id)
        : [...prev, friend]
    );
  };
  ...
  const handleSend = () => {
    console.log('handleSend called in NewMessageModal');
    ...
    const finalGroupName = createGroup && selectedRecipients.length > 1 && groupName.trim() 
      ? groupName.trim() 
      : "";
    console.log('finalGroupName:', finalGroupName);
    console.log('About to call onSend...');
    onSend(selectedRecipients, message.trim(), createGroup && selectedRecipients.length > 1, finalGroupName);
    console.log('onSend called successfully');
  };

  return (
    <div style={{ position: "fixed", inset: 0, background: T.overlay, zIndex: 110, display: "flex", alignItems: "flex-end", animation: "fadeIn 0.2s" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.card, width: "100%", maxWidth: 480, margin: "0 auto", borderRadius: "24px 24px 0 0", maxHeight: "75vh", display: "flex", flexDirection: "column", animation: "slideUp 0.3s ease-out" }}>
        <div style={{ padding: "18px 20px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            {step === "compose" && (
              <button onClick={() => setStep("select")} style={{ background: "none", border: "none", cursor: "pointer", padding: 6, display: "flex" }}>
                <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={T.textSec} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="15 18 9 12 15 6"></polyline>
                </svg>
              </button>
            )}
            <div style={{ fontSize: 18, fontWeight: 800, color: T.text }}>
              {step === "select" ? "New Message" : "Write Message"}
            </div>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", padding: 6 }}>
            <X size={22} color={T.textSec} strokeWidth={2.2} />
        ...
        {step === "select" ? (
          <>
            <div style={{ flex: 1, overflow: "auto", padding: "16px 20px" }}>
              ...
              {FRIENDS.map(friend => {
                const selected = selectedRecipients.find(r => r.id === friend.id);
                return (
                  <div
                    key={friend.id}
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: T.textTer }}>{friend.bio}</div>
                    </div>
                    <div style={{
                      width: 22, height: 22, borderRadius: "50%",
                      background: selected ? T.accent : "transparent",
                      border: `2px solid ${selected ? T.accent : T.borderMed}`,
            ...
            <div style={{ padding: "16px 20px", borderTop: `1px solid ${T.border}` }}>
              <button
                ...
                style={{
                  width: "100%",
                  padding: "14px",
                  background: selectedRecipients.length > 0 ? T.accent : T.border,
                  border: "none",
                  borderRadius: 12,
                  color: "#fff",
                  fontWeight: 700,
              ...
              >
                {selectedRecipients.length === 0 ? "Select at least one person" : "Next"}
              </button>
            </div>
          </>
        ) : (
          <>
            <div style={{ padding: "16px 20px", borderBottom: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: T.textSec, marginBottom: 8 }}>To:</div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {selectedRecipients.map(recipient => (
                  <div key={recipient.id} style={{ display: "flex", alignItems: "center", gap: 6, background: T.accentLight, borderRadius: 20, padding: "4px 10px 4px 4px", border: `1px solid ${T.accent}` }}>
                    <Avatar user={recipient} size={20} />
                    <span style={{ fontSize: 12, fontWeight: 600, color: T.accent }}>{recipient.name}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Group creation option for multiple recipients */}
            {selectedRecipients.length > 1 && (
              <div style={{ padding: "0 20px 16px" }}>
                <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", marginBottom: createGroup ? 10 : 0 }}>
                  <input
                    type="checkbox"
                    checked={createGroup}
                  ...
                  />
                  <span style={{ fontSize: 13, fontWeight: 600, color: T.text }}>Save as group</span>
                </label>
                {createGroup && (
                  <input
                    value={groupName}
            ...
            <div style={{ padding: "16px 20px", borderTop: `1px solid ${T.border}` }}>
              ...
              >
                <Send size={16} />
                {selectedRecipients.length === 1
                  ? `Send to ${selectedRecipients[0].name}`
...
// --- Group Detail Modal ---
function GroupDetailModal({ group, onClose, onDelete, T, userDMs, onShowDM }) {
  const [activeTab, setActiveTab] = useState("members"); // "members" | "messages"
  const [messageText, setMessageText] = useState("");

  // Gather messages from all group members
  const groupMessages = group.members.flatMap(member => {
    const msgs = userDMs?.[member.id] || [];
    return msgs.map(msg => ({ ...msg, memberName: member.name, memberColor: member.color, memberId: member.id }));
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: T.card, width: "100%", maxWidth: 480, margin: "0 auto", borderRadius: "24px 24px 0 0", maxHeight: "85vh", display: "flex", flexDirection: "column", animation: "slideUp 0.3s ease-out" }}>
        ...
        <div style={{ padding: "18px 20px 0", borderBottom: `1px solid ${T.border}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
            <div>
              <div style={{ fontSize: 20, fontWeight: 800, color: T.text, letterSpacing: -0.4 }}>{group.name}</div>
              <div style={{ fontSize: 13, color: T.textTer, marginTop: 2 }}>{group.members.length} members</div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={() => onDelete(group.id)}
                style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 10, padding: "6px 10px", cursor: "pointer", color: T.red, fontSize: 12, fontWeight: 600 }}
              >
                Delete
              </button>
              <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", padding: 6 }}>
        ...
        {activeTab === "members" && (
          <div style={{ flex: 1, overflow: "auto", padding: "16px 20px" }}>
            {group.members.map((member, i) => (
              <div
                key={member.id}
                style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: `1px solid ${T.border}`, animation: `fadeIn 0.3s ease-out ${i * 0.05}s backwards` }}
              >
                <Avatar user={member} size={44} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: 14, color: T.text }}>{member.name}</div>
                  <div style={{ fontSize: 12, color: T.textTer }}>{member.bio}</div>
                </div>
                <button
                  onClick={() => { onShowDM && onShowDM(member); onClose(); }}
                  style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 10, padding: "6px 12px", cursor: "pointer", fontSize: 12, fontWeight: 600, color: T.text, display: "flex", alignItems: "center", gap: 5 }}
                >
                  <MessageCircle size={13} color={T.accent} /> Message
                </button>
              </div>
            ))}
          </div>
        ...
        {activeTab === "messages" && (
          <div style={{ flex: 1, overflow: "auto", padding: "16px 20px" }}>
            {groupMessages.length === 0 ? (
              <div style={{ textAlign: "center", padding: "48px 0", color: T.textTer }}>
                <MessageCircle size={40} color={T.borderMed} strokeWidth={1.2} />
                <div style={{ fontSize: 14, fontWeight: 600, marginTop: 12 }}>No messages yet</div>
                <div style={{ fontSize: 12, marginTop: 4 }}>Messages you've sent to group members will appear here</div>
              </div>
            ) : (
              groupMessages.map((msg, i) => (
                <div key={i} style={{ marginBottom: 12, display: "flex", flexDirection: msg.from === "me" ? "row-reverse" : "row", alignItems: "flex-end", gap: 8 }}>
                  {msg.from !== "me" && (
                    <div style={{ width: 28, height: 28, borderRadius: "50%", background: msg.memberColor, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>
                      {msg.memberName?.[0]}
                    </div>
                  )}
                  <div style={{ maxWidth: "70%" }}>
                    {msg.from !== "me" && <div style={{ fontSize: 10, fontWeight: 600, color: T.textTer, marginBottom: 3 }}>{msg.memberName}</div>}
                    <div style={{ background: msg.from === "me" ? T.accent : T.bg, color: msg.from === "me" ? "#fff" : T.text, padding: "8px 12px", borderRadius: msg.from === "me" ? "16px 16px 4px 16px" : "16px 16px 16px 4px", fontSize: 13, border: msg.from === "me" ? "none" : `1px solid ${T.border}`, lineHeight: 1.45 }}>
                      {msg.text}
                    </div>
                    <div style={{ fontSize: 10, color: T.textTer, marginTop: 3, textAlign: msg.from === "me" ? "right" : "left" }}>
                      {new Date(msg.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </div>
                  </div>
                </div>
...
function CreateGroupModal({ onClose, onCreate, T }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: T.overlay, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", animation: "fadeIn 0.2s" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.card, width: "90%", maxWidth: 420, borderRadius: 20, padding: "24px", animation: "slideUp 0.3s ease-out", maxHeight: "85vh", overflow: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: T.text }}>Create Group</div>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", padding: 6 }}>
            <X size={22} color={T.textSec} strokeWidth={2.2} />
          </button>
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: T.textSec, marginBottom: 8, display: "block" }}>Group Name</label>
          <input
            value={name}
            onChange={e => setName(e.target.value)}
        ...
        <div style={{ marginBottom: 20 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: T.textSec, marginBottom: 10, display: "block" }}>
            Add Members ({selectedMembers.length} selected)
          </label>
          {FRIENDS.map(friend => {
            const selected = selectedMembers.find(m => m.id === friend.id);
            return (
              <div
                key={friend.id}
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button
            ...
            style={{ flex: 1, padding: "12px", background: name.trim() && selectedMembers.length > 0 ? T.accent : T.border, border: "none", borderRadius: 12, color: "#fff", fontWeight: 700, fontSize: 14, cursor: name.trim() && selectedMembers.length > 0 ? "pointer" : "not-allowed", opacity: name.trim() && selectedMembers.length > 0 ? 1 : 0.5 }}
          >
            Create Group
          </button>
        </div>
...
function ProfileTab({ checkins, stats, T, darkMode, userCollections, onMovieClick, onEditCheckin, onDeleteCheckin, onAddToCollection, onRemoveFromCollection, onEditCollection, onDeleteCollection, onEdit, userName, onEditName, ME, userDMs, onShowDM, onSendDM }) {
  ...
  const [groups, setGroups] = useState([
    { id: "g1", name: "Movie Night Crew", members: [FRIENDS[0], FRIENDS[1], FRIENDS[2]], createdAt: Date.now() },
  ]);
  ...
  // Helper: check if group has any messages
  const hasGroupMessages = (group, dms) => {
    if (!dms) return false;
    return group.members.some(member => dms[member.id] && dms[member.id].length > 0);
  };

  // Helper: get all messages from group members
  const getGroupMessages = (group, dms) => {
    if (!dms) return [];
    return group.members.flatMap(member => {
      const msgs = dms[member.id] || [];
      return msgs.map(msg => ({ ...msg, memberName: member.name, memberId: member.id }));
    }).sort((a, b) => a.timestamp - b.timestamp);
  };
  ...
  return (
    ...
    <div>
      ...
      {subTab === "collections" && (
        <div>
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ color: T.text, fontSize: 14, fontWeight: 700 }}>My Collections</div>
              <button
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
              {userCollections.map((collection, i) => (
                <div
                  ...
                  onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}
                >
                  <div onClick={() => setSelectedCollection(collection)} style={{ cursor: "pointer" }}>
                    <div style={{ width: "100%", aspectRatio: "16/9", background: T.bg, borderRadius: 10, marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "center", border: `1px solid ${T.border}` }}>
                      <FilmIcon size={32} color={T.textTer} />
                    </div>
                    <div style={{ fontWeight: 700, fontSize: 14, color: T.text, marginBottom: 4 }}>{collection.name}</div>
      ...
      {subTab === "friends" && (
        <div>
          ...
          <div style={{ marginBottom: 24 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ color: T.text, fontSize: 14, fontWeight: 700 }}>Groups ({groups.length})</div>
              <button
                onClick={() => setShowCreateGroup(true)}
                style={{ padding: "7px 12px", background: T.accent, border: "none", borderRadius: 10, color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}
              >
                <Plus size={14} /> New Group
              </button>
            </div>
            {groups.length === 0 ? (
              <div style={{ textAlign: "center", padding: "32px 0", color: T.textTer }}>
                <Users size={36} color={T.borderMed} strokeWidth={1.2} />
                <div style={{ fontSize: 14, fontWeight: 600, marginTop: 12 }}>No groups yet</div>
                <div style={{ fontSize: 12, marginTop: 4 }}>Create a group to share movies with multiple friends</div>
              </div>
            ) : (
              groups.map((group, i) => (
                <div
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                    ...
                    >
                      ...
                      <div style={{ fontSize: 12, color: T.textTer, marginTop: 2 }}>{group.members.length} members</div>
                    </div>
                    <button
                      onClick={() => setDeletingGroup(group)}
                      style={{ background: "none", border: "none", cursor: "pointer", padding: 6, color: T.textTer }}
                    >
                      <X size={16} />
                    </button>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {group.members.map(member => (
                      <div key={member.id} style={{ display: "flex", alignItems: "center", gap: 6, background: T.bg, borderRadius: 20, padding: "4px 10px 4px 4px", border: `1px solid ${T.border}` }}>
      ...
      {subTab === "messages" && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: T.text }}>Messages</div>
            <button
          ...
          {/* Individual DMs */}
          {Object.keys(userDMs || {}).length === 0 && groups.filter(g => hasGroupMessages(g, userDMs)).length === 0 ? (
            <div style={{ textAlign: "center", padding: 56, color: T.textTer }}>
            ...
            <>
              ...
              {/* Group conversations */}
              {groups.filter(g => hasGroupMessages(g, userDMs)).map(group => {
                const groupMessages = getGroupMessages(group, userDMs);
                ...
                return (
                  <div
                    ...
                    onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}
                  >
                    <div style={{ width: 48, height: 48, borderRadius: "50%", background: T.accentLight, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <Users size={22} color={T.accent} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: 14, color: T.text, marginBottom: 2 }}>{group.name}</div>
                      <div style={{ fontSize: 12, color: T.textTer, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {lastMsg ? lastMsg.text : `${group.members.length} members`}
                      </div>
                    </div>
                    {lastMsg && (
                      <div style={{ fontSize: 10, color: T.textTer, whiteSpace: "nowrap" }}>
    ...
    {deletingGroup && (
      <div style={{ position: "fixed", inset: 0, background: T.overlay, zIndex: 120, display: "flex", alignItems: "center", justifyContent: "center", animation: "fadeIn 0.2s" }} onClick={() => setDeletingGroup(null)}>
        <div onClick={e => e.stopPropagation()} style={{ background: T.card, width: "90%", maxWidth: 340, borderRadius: 20, padding: "24px", animation: "slideUp 0.3s ease-out" }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: T.text, marginBottom: 10 }}>Delete Group?</div>
          <div style={{ fontSize: 14, color: T.textSec, marginBottom: 24, lineHeight: 1.5 }}>
            Are you sure you want to delete <span style={{ fontWeight: 700, color: T.text }}>"{deletingGroup.name}"</span>? This cannot be undone.
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => setDeletingGroup(null)} style={{ flex: 1, padding: "12px", background: T.bg, border: `1.5px solid ${T.border}`, borderRadius: 12, color: T.text, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>Cancel</button>
            <button
              onClick={() => {
                setGroups(prev => prev.filter(g => g.id !== deletingGroup.id));
                setDeletingGroup(null);
              }}
              style={{ flex: 1, padding: "12px", background: T.red, border: "none", borderRadius: 12, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer" }}
            >Delete</button>
          </div>
        </div>
      </div>
    ...
    {showNewMessage && (
      <NewMessageModal
        ...
        onSend={(recipients, messageText, shouldCreateGroup, groupName) => {
          console.log('=== NEW MESSAGE SEND ===');
          console.log('Recipients:', recipients);
          console.log('Message:', messageText);
          console.log('Should create group:', shouldCreateGroup);
          ...
          recipients.forEach((recipient, index) => {
            const newMessage = {
              id: `msg-${Date.now()}-${index}-${recipient.id}`,
              text: messageText,
              from: "me",
              timestamp: Date.now() + index
            };
            console.log(`Sending message ${index + 1}/${recipients.length} to ${recipient.name}:`, newMessage);
            if (onSendDM) {
              onSendDM(recipient.id, newMessage);
              messageCount++;
            } else {
              console.error('onSendDM is not available!');
            }
          });
          console.log(`Total messages sent: ${messageCount}`);
          
          // Create group if requested and multiple recipients
          if (shouldCreateGroup && recipients.length > 1 && groupName) {
            const newGroup = {
              id: `grp-${Date.now()}`,
              name: groupName,
              members: recipients,
          ...
          // Open DM with first recipient after a brief delay to let state update
          setTimeout(() => {
            if (recipients.length > 0 && onShowDM) {
              onShowDM(recipients[0]);
            }
          }, 100);
        }}
        T={T}
...
// --- Movie Detail Modal ---
function MovieDetailModal({ movie, onClose, userCollections, onAddToCollection, onRemoveFromCollection, T, checkins, onShare, shareGroups }) {
  const [showCollectionPicker, setShowCollectionPicker] = useState(false);
  const [showWatchHistory, setShowWatchHistory] = useState(false);
  const [showShareModal, setShowShareModal] = useState(false);
  const [sharingWith, setSharingWith] = useState(null); // Friend being shared with
  const [shareMessage, setShareMessage] = useState(""); // Custom message
  const [fullDetails, setFullDetails] = useState(null);
  const [loadingDetails, setLoadingDetails] = useState(false);

  // Fetch full details when modal opens (if we have tmdbId)
  useEffect(() => {
    const fetchDetails = async () => {
      if (movie?.tmdbId && !fullDetails) {
    ...
    <div style={{ position: "fixed", inset: 0, background: T.overlay, zIndex: 100, display: "flex", alignItems: "flex-end", animation: "fadeIn 0.2s" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.card, width: "100%", maxWidth: 480, margin: "0 auto", borderRadius: "24px 24px 0 0", maxHeight: "90vh", overflow: "hidden", display: "flex", flexDirection: "column", animation: "slideUp 0.3s ease-out" }}>
        ...
        <div style={{ padding: "18px 20px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div style={{ flex: 1, paddingRight: 12 }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: T.text, marginBottom: 4, letterSpacing: -0.4 }}>{displayMovie.title}</div>
            <div style={{ fontSize: 13, color: T.textTer }}>{displayMovie.year}</div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button 
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "20px" }}>
          ...
          <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
            <div style={{ width: 100, height: 150, borderRadius: 12, background: T.bg, border: `1px solid ${T.border}`, flexShrink: 0, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {displayMovie.poster ? (
                <img src={displayMovie.poster} alt={displayMovie.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              ) : (
                <FilmIcon size={32} color={T.textTer} />
              )}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 24, fontWeight: 800, color: T.text }}>{displayMovie.rating}</span>
              ...
              {/* Streaming Info */}
              {(displayMovie.streamingOn?.length > 0 || displayMovie.rentOn?.length > 0 || displayMovie.buyOn?.length > 0) && (
                <div style={{ marginTop: 8, padding: "8px 10px", background: T.bg, borderRadius: 8, fontSize: 11 }}>
                  {displayMovie.streamingOn && displayMovie.streamingOn.length > 0 && (
                    <div style={{ marginBottom: 4 }}>
                      <span style={{ fontWeight: 700, color: T.text }}>Stream: </span>
                      <span style={{ color: T.textSec }}>{displayMovie.streamingOn.join(', ')}</span>
                    </div>
                  )}
                  {displayMovie.rentOn && displayMovie.rentOn.length > 0 && (
                    <div style={{ marginBottom: 4 }}>
                      <span style={{ fontWeight: 700, color: T.text }}>Rent: </span>
                      <span style={{ color: T.textSec }}>{displayMovie.rentOn.slice(0, 3).join(', ')}</span>
                    </div>
                  )}
                  {displayMovie.buyOn && displayMovie.buyOn.length > 0 && (
                    <div>
                      <span style={{ fontWeight: 700, color: T.text }}>Buy: </span>
                      <span style={{ color: T.textSec }}>{displayMovie.buyOn.slice(0, 3).join(', ')}</span>
                    </div>
                  )}
                </div>
              )}
          ...
          {displayMovie.overview && (
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 14, color: T.textSec, lineHeight: 1.6 }}>{displayMovie.overview}</div>
            </div>
          )}
          
          {/* Director and Cast */}
          {(displayMovie.director || (displayMovie.cast && displayMovie.cast.length > 0)) && (
            <div style={{ marginBottom: 20, padding: "12px 14px", background: T.bg, borderRadius: 12 }}>
              {displayMovie.director && (
                <div style={{ marginBottom: displayMovie.cast && displayMovie.cast.length > 0 ? 8 : 0 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: T.text }}>Directed by: </span>
                  <span style={{ fontSize: 12, color: T.textSec }}>{displayMovie.director}</span>
                </div>
              )}
              {displayMovie.cast && displayMovie.cast.length > 0 && (
                <div>
                  <span style={{ fontSize: 12, fontWeight: 700, color: T.text }}>Starring: </span>
                  <span style={{ fontSize: 12, color: T.textSec }}>{displayMovie.cast.join(', ')}</span>
                </div>
              )}
            </div>
          )}
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            {userCollections.filter(col => col.movies.some(m => m.id === displayMovie.id)).length > 0 ? (
              <div style={{ marginBottom: 12 }}>
                {userCollections.filter(col => col.movies.some(m => m.id === displayMovie.id)).map(col => (
                  <div key={col.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", background: T.bg, borderRadius: 10, marginBottom: 8 }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: T.text }}>{col.name}</div>
                      <div style={{ fontSize: 11, color: T.textTer }}>{col.movieCount} movies</div>
                    ...
                    <button
                      onClick={() => onRemoveFromCollection(displayMovie.id, col.id)}
                      style={{ padding: "6px 12px", background: "none", border: `1px solid ${T.border}`, borderRadius: 8, color: T.textSec, fontSize: 11, fontWeight: 600, cursor: "pointer" }}
                    >
                      Remove
                    </button>
        ...
        {/* Share Modal */}
        {showShareModal && (
          <div style={{ position: "fixed", inset: 0, background: T.overlay, zIndex: 110, display: "flex", alignItems: "flex-end", animation: "fadeIn 0.2s" }} onClick={() => { setShowShareModal(false); setSharingWith(null); setShareMessage(""); }}>
            <div onClick={e => e.stopPropagation()} style={{ background: T.card, width: "100%", maxWidth: 480, margin: "0 auto", borderRadius: "24px 24px 0 0", maxHeight: "70vh", overflow: "hidden", display: "flex", flexDirection: "column", animation: "slideUp 0.3s ease-out" }}>
              <div style={{ padding: "18px 20px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  {sharingWith && (
                    <button 
                      onClick={() => { setSharingWith(null); setShareMessage(""); }}
                  ...
                  <div style={{ fontSize: 18, fontWeight: 800, color: T.text }}>
                    {sharingWith ? `Share with ${sharingWith.name}` : "Share with Friends"}
                  </div>
                </div>
                <button onClick={() => { setShowShareModal(false); setSharingWith(null); setShareMessage(""); }} style={{ background: "none", border: "none", cursor: "pointer", padding: 6 }}>
                  <X size={22} color={T.textSec} strokeWidth={2.2} />
                </button>
              </div>
              ...
              {!sharingWith ? (
                ...
                <div style={{ flex: 1, overflow: "auto", padding: "20px" }}>
                  {/* Friends */}
                  <div style={{ fontSize: 12, fontWeight: 700, color: T.textTer, marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.5 }}>Friends</div>
                  ...
                  {shareGroups && shareGroups.length > 0 && (
                    <>
                      <div style={{ fontSize: 12, fontWeight: 700, color: T.textTer, margin: "16px 0 8px", textTransform: "uppercase", letterSpacing: 0.5 }}>Groups</div>
                      {shareGroups.map((group, i) => (
                        <div
                          key={group.id}
                          onClick={() => setSharingWith({ ...group, isGroup: true })}
                          style={{
                            background: T.bg,
                            border: `1px solid ${T.border}`,
                            borderRadius: 12,
                          ...
                          onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}
                        >
                          <div style={{ width: 42, height: 42, borderRadius: "50%", background: T.accentLight, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                            <Users size={20} color={T.accent} />
                          </div>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontWeight: 700, fontSize: 14, color: T.text }}>{group.name}</div>
                            <div style={{ fontSize: 12, color: T.textTer }}>{group.members.length} members · {group.members.map(m => m.name).join(', ')}</div>
                          </div>
                          <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={T.textSec} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
                            <polyline points="9 18 15 12 9 6"></polyline>
                          </svg>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "20px" }}>
                  {/* Movie preview */}
                  <div style={{ display: "flex", gap: 12, padding: "12px 14px", background: T.bg, borderRadius: 12, marginBottom: 16 }}>
                    <div style={{ width: 50, height: 75, borderRadius: 8, background: T.card, border: `1px solid ${T.border}`, flexShrink: 0, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {displayMovie.poster ? (
                        <img src={displayMovie.poster} alt={displayMovie.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      ) : (
                        <FilmIcon size={20} color={T.textTer} />
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: T.textTer }}>
                        {displayMovie.year}
                        {displayMovie.rating && ` • ${displayMovie.rating}/10`}
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Message input */}
                  <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: T.textSec, marginBottom: 8 }}>Add a message (optional)</div>
                    <textarea
                      value={shareMessage}
                      onChange={e => setShareMessage(e.target.value)}
                      placeholder="Why do you think they'll love this?"
                      autoFocus
                      style={{ 
                        flex: 1,
                        minHeight: 100,
                  ...
                  <button
                    onClick={() => {
                      if (sharingWith?.isGroup) {
                        // Send to all group members
                        sharingWith.members.forEach(member => {
                          if (onShare) onShare(displayMovie, member, shareMessage);
                        });
                      } else {
                        if (onShare) onShare(displayMovie, sharingWith, shareMessage);
                      }
                  ...
                  >
                    <Send size={16} />
                    {sharingWith?.isGroup 
                      ? `Send to ${sharingWith.name} (${sharingWith.members.length} members)`
                      : `Send to ${sharingWith?.name}`
                    }
                  </button>
                </div>
              )}
...
export default function SeenApp() {
  ...
  function handleSendDM(friendId, message) {
    console.log(`handleSendDM called - friendId: ${friendId}, message:`, message);
    setUserDMs(prev => {
      const updated = {
        ...prev,
        [friendId]: [...(prev[friendId] || []), message]
      };
      console.log('Updated userDMs:', updated);
      console.log(`Total conversations: ${Object.keys(updated).length}`);
      // Persist to storage
      if (window.storage) {
        window.storage.set('seen-dms', JSON.stringify(updated)).catch(err => 
          console.log('Failed to save DMs:', err)
        );
      }
      return updated;
    });
  ...
  function handleDeleteMessage(friendId, messageId) {
    setUserDMs(prev => ({
      ...prev,
  ...
  function handleShareMovie(movie, friend, customMessage = "") {
    const defaultMessage = `Check out "${movie.title}" (${movie.year})! ${movie.rating ? `I'd rate it ${movie.rating}/10` : ''}`;
    const message = {
      id: `msg-${Date.now()}`,
  ...
  const editCollection = (updated) => {
    setUserCollections(prev => {
      const exists = prev.find(col => col.id === updated.id);
      if (exists) {
        // Edit existing
        return prev.map(col => col.id === updated.id ? updated : col);
      } else {
        // Create new
        return [updated, ...prev];
      }
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div style={{ padding: "14px 14px 100px" }}>
            ...
            {tab === "profile" && <ProfileTab checkins={my} stats={stats} T={T} darkMode={darkMode} userCollections={userCollections} onMovieClick={setSelectedMovieDetail} onEditCheckin={handleEditCheckin} onDeleteCheckin={handleDeleteCheckin} onAddToCollection={addToCollection} onRemoveFromCollection={removeFromCollection} onEditCollection={editCollection} onDeleteCollection={deleteCollection} onEdit={setEditingCheckin} userName={userName} onEditName={() => setShowEditProfile(true)} ME={ME} userDMs={userDMs} onShowDM={(friend) => { setShowDM(friend); setTab("profile"); }} onSendDM={handleSendDM} />}
          </div>
          ...
          {/* DM Modal */}
          {showDM && (
            <DMModal user={showDM} onClose={() => setShowDM(null)} T={T} userDMs={userDMs} onSendDM={handleSendDM} onDeleteMessage={handleDeleteMessage} />
          )}
